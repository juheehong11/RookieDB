...
public class TestRecoveryManager {
    ...
    /*
    pgsize: 4096
    dpt 16 bytes each    4096/16 = 256 dpt entries in the first checkpt   (1000 - 256)/16 entries in the 2nd checkpoint 
    txn table entry is 17 bytes each  

    first 3: 256 dpt entries and the 4th one is 22 txn tbl entries + 232 dpt entries 
    1000-22 = # of txn entries left

    981 left
    4096 bytes / 17 each time
    240 txn tbl entries each round 


    last round 18 txn tbl entries 
    */
    @Test
    @Category(PublicTests.class)
    public void testCheckpoint1000() {
        // Create 1000 DPT entries and 1000 transaction table entries
        ...
        Iterator<LogRecord> logs = logManager.scanFrom(10000L); //how do we know where to scan from?

        // Next 3 logs should be from the checkpoint 
        //--> how do we know how many of the next logs should be from the checkpoint, and how many end checkpoints we need?
        //how do we know when the checkpoint ends / how do we indicate that?
        LogRecord beginCheckpoint = logs.next();
        LogRecord endCheckpoint1 = logs.next();
        LogRecord endCheckpoint2 = logs.next();
        ...
        LogRecord endCheckpoint8 = logs.next();
        LogRecord endCheckpoint9 = logs.next();
        assertEquals(LogType.BEGIN_CHECKPOINT, beginCheckpoint.getType());
        assertEquals(LogType.END_CHECKPOINT, endCheckpoint1.getType());
        assertEquals(LogType.END_CHECKPOINT, endCheckpoint2.getType());
        ...
        assertEquals(LogType.END_CHECKPOINT, endCheckpoint8.getType());
        assertEquals(LogType.END_CHECKPOINT, endCheckpoint9.getType());
        assertFalse(logs.hasNext());
        ...
        assertEquals(235, endCheckpoint4.getDirtyPageTable().size());
        assertEquals(19, endCheckpoint4.getTransactionTable().size());
        ...
        assertEquals(240, endCheckpoint7.getTransactionTable().size());

        assertEquals(0, endCheckpoint8.getDirtyPageTable().size());
        assertEquals(240, endCheckpoint8.getTransactionTable().size());

        assertEquals(0, endCheckpoint9.getDirtyPageTable().size());
        assertEquals(21, endCheckpoint9.getTransactionTable().size());

        // Check the contents of the checkpoint DPT/transaction tables match
        // what we inserted earlier

        for (long l = 1; l <= 1000; l++) {
            //assertEquals(l*l, (long) endCheckpoint1.getDirtyPageTable().get(l));// --> is this line really needed?
            if (endCheckpoint1.getDirtyPageTable().containsKey(l)) {
                assertEquals(l*l, (long) endCheckpoint1.getDirtyPageTable().get(l));
            } else if (endCheckpoint2.getDirtyPageTable().containsKey(l)) {
                assertEquals(l*l, (long) endCheckpoint2.getDirtyPageTable().get(l));
            } else if (endCheckpoint3.getDirtyPageTable().containsKey(l)) {
                assertEquals(l*l, (long) endCheckpoint3.getDirtyPageTable().get(l));
            } else if (endCheckpoint4.getDirtyPageTable().containsKey(l)) {
                assertEquals(l*l, (long) endCheckpoint4.getDirtyPageTable().get(l));
            } 

            Pair<Transaction.Status, Long> p;
            if (endCheckpoint4.getTransactionTable().containsKey(l)) {
            ...
            } else if (endCheckpoint8.getTransactionTable().containsKey(l)) {
                p = endCheckpoint8.getTransactionTable().get(l);
            } else if (endCheckpoint9.getTransactionTable().containsKey(l)) {
                p = endCheckpoint9.getTransactionTable().get(l);
            } else {
                fail("Transaction #" + l + " could not be found in checkpoint ttables");
                return;
            }
